...
function App() {
  ...
  const [dashboardStatus, setDashboardStatus] =
    useState<DashboardAuthStatus | null>(null);
  const [dashboardToken, setDashboardToken] = useState<string | null>(
    getAuthToken(),
  );
  const [dashboardBusy, setDashboardBusy] = useState(false);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  const [mainView, setMainView] = useState<"sessions" | "scripting">(
    "sessions",
  );
  ...
  const [sessionInputs, setSessionInputs] = useState<
    Record<string, SessionInputs>
  >({});
  const [minimaps, setMinimaps] = useState<
    Record<string, MinimapSnapshot | null>
  >({});
  const [luaStatuses, setLuaStatuses] = useState<
    Record<string, LuaScriptStatusSnapshot | null>
  >({});
  const [autonetherStatuses, setAutonetherStatuses] = useState<
    Record<string, { active: boolean; phase: string } | null>
  >({});
  ...
  const [automineSpeed, setAutomineSpeedState] = useState<
  ...
  const refreshDashboardStatus = useCallback(async () => {
    try {
      ...
      if (appStatus.config?.maintenance) {
        setMaintenance({
          active: true,
          message: appStatus.config.maintenance_message,
        });
      }
      ...
      setFeedback({
        kind: "error",
        message:
          error instanceof Error ? error.message : "dashboard auth failed",
      });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      socket.onopen = () => {
        reconnectDelay = 1000;
        setFeedback((current) =>
          current?.message === "dashboard websocket disconnected"
            ? null
            : current,
        );
      };
  ...
  const refreshMinimap = useCallback(
    async (sessionId: string, currentWorld: string | null) => {
  ...
  const hoverChangeBySessionRef = useRef<
    Record<string, (value: string) => void>
  >({});

  const getHoverChange = useCallback(
    (sessionId: string) => {
  ...
  const handleConnect = async () => {
    setConnecting(true);
    try {
      const response = await connectWithAuth(
        createAuthInput,
        proxy || undefined,
      );
      updateFromAction(response);
  ...
  const handleCreateCode = async () => {
    ...
    try {
      ...
      setFeedback({
        kind: "success",
        message: "Code created! Redirecting soon...",
      });
            ...
            <CardContent className="grid gap-6">
              {dashboardCode ? (
                <div className="flex flex-col items-center gap-4 py-8">
                  <div className="text-xs text-muted-foreground uppercase tracking-[0.3em] font-bold">
                    Your 6-Digit Code
                  </div>
                  <div className="relative">
                  ...
                  <div className="text-[10px] text-muted-foreground/60 mt-4 text-center max-w-sm">
                    Code will be saved to{" "}
                    <span className="text-primary font-mono">code.txt</span>
                  </div>
                </div>
              ) : (
                <>
        ...
        <Card className="max-w-md w-full border-white/10 bg-card/50 backdrop-blur-3xl ring-1 ring-white/10 hydro-glow-lg">
          ...
          <CardContent className="space-y-6 pt-4">
            ...
            <div className="space-y-2 text-center text-xs text-muted-foreground">
              <p>
                Please check back later. We are working hard to bring you the
                best experience.
              </p>
  ...
  return (
    <div className="min-h-screen bg-[#0B0D0C] text-foreground selection:bg-primary/30 relative overflow-hidden">
      ...
      <div className="flex h-screen relative z-10">
        {/* Left Sidebar */}
        <div
          className={`flex flex-col border-r border-border bg-[var(--sidebar)] backdrop-blur-xl transition-all duration-300 z-50 relative ${sidebarCollapsed ? "w-16 items-center" : "w-80"}`}
        >
          {/* Logo & Actions Header */}
          <div
            className={`flex items-center p-4 border-b border-border h-16 w-full ${sidebarCollapsed ? "justify-center" : "justify-between"}`}
          >
            {sidebarCollapsed ? (
              ...
              <>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    onClick={() =>
                      setMainView(
                        mainView === "scripting" ? "sessions" : "scripting",
                      )
                    }
          ...
          <div className="group relative w-full border-b border-border">
            ...
            >
              ...
              {!sidebarCollapsed && (
                <CaretRight
                  className={`size-4 text-muted-foreground transition-transform ${createSessionExpanded ? "rotate-90" : ""}`}
                />
              )}
              {sidebarCollapsed && (
                <CaretRight className="size-3 text-muted-foreground hidden group-hover:block absolute right-1" />
              )}
            </div>
          ...
          <div className="group relative w-full border-b border-border flex-1 flex flex-col min-h-0">
            ...
            >
              ...
              {!sidebarCollapsed && (
                <h2 className="font-semibold text-sm truncate flex-1">Bots</h2>
              )}
              {sidebarCollapsed && (
                <CaretRight className="size-3 text-muted-foreground hidden group-hover:block absolute right-1" />
              )}
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="text-[10px] text-muted-foreground truncate">
                        {session.username ?? session.user_id ?? "No user"}
                      </div>
                    </button>
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-4 mb-3">
                    ...
                    <h1 className="text-4xl font-extrabold tracking-tight text-white drop-shadow-md flex items-center gap-3">
                      Hydro{" "}
                      <span className="text-primary drop-shadow-[0_0_10px_rgba(var(--primary),0.5)]">
                        Dashboard
                      </span>
                  ...
                  <p className="text-base text-white/70 max-w-xl leading-relaxed">
                    Welcome to your central command. Manage your bot sessions,
                    configure automation parameters, and write powerful Lua
                    scripts to dominate your world.
                  </p>
                ...
                {sessions.length ? (
                                ...
                                <div className="grid gap-1">
                                  ...
                                  <CardDescription>
                                    ...
                                    {session.username ??
                                      session.user_id ??
                                      "-"}{" "}
                                    world={session.current_world ?? "-"} host=
                                    {session.current_host}:
                                    {session.current_port}
                                  </CardDescription>
                                  <div className="text-xs text-muted-foreground">
                                    position map=(
                                    {session.player_position.map_x ?? "-"},{" "}
                                    {session.player_position.map_y ?? "-"})
                                    world=(
                                    {session.player_position.world_x ??
                                      "-"},{" "}
                                    {session.player_position.world_y ?? "-"})
                                    ping=
                                    {session.ping_ms != null
                                      ? `${session.ping_ms}ms`
                                      : "-"}
                                  </div>
                                  {session.last_error ? (
                            ...
                            <CardContent className="grid gap-4">
                              ...
                              <Tabs defaultValue="world" className="w-full">
                                <TabsList className="grid w-full grid-cols-3 rounded-xl border border-white/10 bg-black/40 p-1.5 shadow-inner">
                                  <TabsTrigger
                                    value="world"
                                    className="rounded-lg data-[state=active]:bg-primary/20 data-[state=active]:text-primary data-[state=active]:shadow-[0_0_15px_-3px_rgba(var(--primary),0.3)] transition-all font-bold"
                                  >
                                    World
                                  </TabsTrigger>
                                  <TabsTrigger
                                ...
                                >
                                  <div className="grid gap-3 rounded-2xl glass-dark p-4">
                                    ...
                                    <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
                                      ...
                                      <Button
                                        ...
                                        onClick={() =>
                                          void runAction(() =>
                                            joinWorld(session.id, inputs.world),
                                          )
                                        }
                                      >
                                        Join
                                  ...
                                  <div className="grid gap-3 rounded-2xl glass-dark p-4">
                                    ...
                                    />
                                    <div className="text-xs text-muted-foreground">
                                      {hoverTiles[session.id] ??
                                        "Hover a tile to inspect it."}
                                    </div>
                                  </div>
                                </TabsContent>
                                ...
                                >
                                  ...
                                  <div className="grid gap-3 rounded-2xl glass-dark p-4">
                                    ...
                                    <div className="text-xs text-muted-foreground mb-2">
                                      Automatically clears the specified world
                                      top-down using pathfinding and smart
                                      mining logic.
                                    </div>
                                  ...
                                  <div className="grid gap-3 rounded-2xl glass-dark p-4">
                                    ...
                                    <div className="text-xs text-muted-foreground">
                                      Adjust mining speed. Lower = safer from
                                      anti-cheat, Higher = faster but riskier.
                                    </div>
                                  ...
                                  <div className="grid gap-3 rounded-2xl glass-dark p-4">
                                    ...
                                    <div className="text-xs text-muted-foreground mb-2">
                                      Automatically enters NETHERWORLD, collects
                                      Nether Keys, and exits through portal.
                                      Requires Nether Scroll in inventory.
                                    </div>
                                  ...
                                  <div className="grid gap-3 rounded-2xl glass-dark p-4">
                                    ...
                                    <div className="text-xs text-muted-foreground mb-2">
                                      Automatically finishes the Growtopia
                                      tutorial. Currently disabled for
                                      maintenance.
                                    </div>
                                    <Button
                                      variant="outline"
                                ...
                                >
                                  ...
                                  <div className="grid gap-3 rounded-2xl glass-dark p-4">
                                    ...
                                    <div className="text-xs text-muted-foreground">
                                      Repeats the same world chat message with
                                      your chosen delay.
                                    </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {session.inventory.length ? (
                                    session.inventory.map((item) => {
                                      const blockName =
                                        blockNames[String(item.block_id)];
                                      const label =
                                        ...
                                        >
                                          ...
                                          <div className="flex gap-1">
                                            <Input
                                              ...
                                              value={
                                                dropAmounts[
                                                  `${session.id}-${item.block_id}-${item.inventory_type}`
                                                ] ?? "1"
                                              }
                                              onChange={(e) => {
                  ...
                  <Card className="border-white/10 bg-card/90 ring-white/10">
                    <CardContent className="px-4 py-8 text-center text-sm text-muted-foreground">
                      No sessions yet. Create a bot using the connection form
                      above.
                    </CardContent>
                  </Card>
                )}
                  ...
                  <div className="flex-1 flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <Button
                        ...
                        onClick={() =>
                          void runLuaAction(activeSessionId, () =>
                            startLuaScript(
                              activeSessionId,
                              sessionInputs[activeSessionId]?.luaSource ?? "",
                            ),
                          )
                        }
                      ...
                      <Button
                        ...
                        onClick={() =>
                          void runAction(() => stopLuaScript(activeSessionId))
                        }
                        disabled={!luaStatuses[activeSessionId]?.running}
                      >
                        Stop Script
                    ...
                    {luaStatuses[activeSessionId]?.last_result_message &&
                      !luaStatuses[activeSessionId]?.last_error && (
                    ...
                    <Card className="border-white/10 bg-card/50 ring-white/10 max-w-md w-full">
                      <CardContent className="px-4 py-8 text-center flex flex-col items-center gap-3">
                        ...
                        <div className="text-sm text-muted-foreground">
                          Please select an active bot session from the sidebar
                          to edit its script.
                        </div>
                      </CardContent>
                    </Card>
                ...
                <div className="space-y-4 text-xs">
                  ...
                  <div className="rounded-lg border border-border bg-black/40 p-3">
                    ...
                    <div className="space-y-2 text-muted-foreground">
                      ...
                      <div>
                        <code className="text-emerald-400">bot:getWorld()</code>{" "}
                        - Get world data
                      </div>
                      <div>
                        <code className="text-emerald-400">
                  ...
                  <div className="rounded-lg border border-border bg-black/40 p-3">
                    ...
                    <div className="space-y-2 text-muted-foreground">
                      ...
                      <div>
                        <code className="text-emerald-400">bot:wear(id)</code> -
                        Wear item
                      </div>
                      <div>
                        <code className="text-emerald-400">bot:unwear(id)</code>{" "}
                        - Unwear item
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-lg border border-border bg-black/40 p-3">
                    ...
                    <div className="space-y-2 text-muted-foreground">
                      <div>
                        <code className="text-emerald-400">bot:collect()</code>{" "}
                        - Collect items
                      </div>
                      <div>
                        <code className="text-emerald-400">
                  ...
                  <div className="rounded-lg border border-border bg-black/40 p-3">
                    ...
                    <div className="space-y-2 text-muted-foreground">
                      <div>
                        <code className="text-emerald-400">bot:sleep(ms)</code>{" "}
                        - Sleep milliseconds
                      </div>
                      <div>
                        <code className="text-emerald-400">bot:talk(msg)</code>{" "}
                        - Send chat message
                      </div>
                      <div>
                        <code className="text-emerald-400">bot:log(msg)</code> -
                        Print to console
                      </div>
                      <div>
                        <code className="text-emerald-400">
                  ...
                  <div className="rounded-lg border border-border bg-black/40 p-3">
                    ...
                    <div className="space-y-2 text-muted-foreground">
                      <div>
                        <code className="text-emerald-400">
                          bot:getDistance(x1,y1,x2,y2)
                        </code>{" "}
                        - Euclidean
                      </div>
                      <div>
                        <code className="text-emerald-400">
                          bot:getManhattanDistance(x1,y1,x2,y2)
                        </code>{" "}
                        - Manhattan
                      </div>
                      <div>
                        <code className="text-emerald-400">
                          bot:isInRange(x1,y1,x2,y2,r)
                        </code>{" "}
                        - Check range
                      </div>
                    </div>
            ...
            <Button
              ...
              onClick={() => {
                ...
                setDashboardStatus((prev) =>
                  prev ? { ...prev, authenticated: false } : null,
                );
              }}
            >
              ...
              onClick={() => {
                try {
                  ...
                  if (!saved) {
                    setFeedback({
                      kind: "error",
                      message: "No saved bot config found.",
                    });
                    return;
                  }
                  ...
                  setFeedback({
                    kind: "error",
                    message: "Failed to load bot config.",
                  });
                }
              }}
            >
            ...
            <Button
              ...
              onClick={() => {
                try {
                  ...
                  setFeedback({
                    kind: "error",
                    message: "Failed to save bot config.",
                  });
                }
              }}
            >
